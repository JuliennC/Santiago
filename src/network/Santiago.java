...
public class Santiago extends UnicastRemoteObject implements SantiagoInterface {
	...
	public String joueurFaitUneOffre() throws RemoteException {
		...
		System.out.println("[" + joueur.getPseudo()
				+ "] : Vous devez faire une enchère : ");

		try {
			offre = scInt.nextInt();
		} catch (InputMismatchException e) {
			return "0_Votre offre est invalide";
		}

		// On regarde la conformité de l'offre
		if (offre < 0) {
			return "0_Vous ne pouvez pas faire une offre inférieur à 0";
		} else if (offre > joueur.getSolde()) {
			return "0_Vous ne pouvez pas faire une offre supérieur à votre solde : "
					+ joueur.getSolde() + ".";
		} else {
			return "1_" + offre;
		}
	...
	public String propositionPhase4() throws RemoteException {
		...
		while (!choixValide) {
			...
			try {
				...
				if (choix < 4 && choix > 0) {
					choixValide = true;
					return ("1_" + choixValide);
				}
			} catch (InputMismatchException ex) {
				System.out.println("Erreur dans la valeur. \n");
				return ("0_Erreur dans la valeur");
			}
		}
		...
		while (!montantValide) {
			...
			try {
				...
				if (montant > 0 && joueur.getSolde() - montant >= 0) {
					...
					return ("0_Votre solde ne peut être inférieur à 0 ! Montant maximum accepté: "
							+ joueur.getSolde() + " Escudos.");
				}
			} catch (InputMismatchException ex) {
				System.out.println("Erreur dans la valeur. \n");
	 ...
	 * @throws JoueurException
	 */
	public void testPartieEstPrete() throws PartieException, RemoteException,
			JoueurException {
		this.verifieServer();
