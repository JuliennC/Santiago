...
public class Santiago extends UnicastRemoteObject implements SantiagoInterface {
	...
	public void verifieServer() {
		if (TYPE.equals("client")) {
			...
			PartieException e = new PartieException("Erreur appelé à un '"
					+ TYPE + "' au lieu d'un server");
			e.printStackTrace();
		}
	}
	...
	/**
	 * Fonction qui vérifie que tous les joueurs sont bien connectés, et avertis
	 * les autres sinon
	 * 
	 * @throws RemoteException
	 */
	public void analyseConnexionJoueurs(final String nom)
			throws RemoteException {
		...
		class SayHello extends TimerTask {
			...
			public void run() {
				...
				if (p == null) {
					System.out.println(p + "Personne dans : " + listeParties);
					return;
				}
	 ...
	 * 
	 * public void addNotificationAPArtie(String nomPartie, int modif) throws
	 * RemoteException{ this.verifieServer();
	 * 
	 * Partie p = getPartieByName(nomPartie); p.addModification(modif); }
	 */

	/**
	 * Fonction qui récupère les parties pour un joueur donné
	 * */
	public ArrayList<Partie> getPartiePourJoueur(String joueur)
			throws RemoteException {
	 ...
	 */
	public boolean joueurParticipeAUnePartie(Joueur joueur,
			ArrayList<Partie> listeParties) throws RemoteException {
	 ...
	 */
	public void poseTuileAvecXY(String nomPartie, Tuile tuile, int x, int y)
			throws RemoteException {
	...
	/**
	 * Fonction appelée par le client pour rejoindre une partie Le client
	 * renseigne le nom de la partie à rejoindre et son objet Joueur
	 * 
	 * @return: booléen de confirmation
	 * @throws PartieException
	 * @throws JoueurException
	 */
	public Partie rejoindrePartie(String nom, SantiagoInterface i)
			throws RemoteException, PartieException, JoueurException {
		System.out.println("coucou");
		this.verifieServer();

		/*
		 * On vérifie d'abord que la liste contient la partie demandée, Sinon,
		 * on lève une exception
		 */
		if (!listeContientPartie(listeParties, nom)) {
			System.out
					.println("Aucune partie contenant ce nom n'est enregistrée");
			return null;
		}
			...
			if (p.getNomPartie().equals(nom)) {
				...
				try {
					...
					if (p.getPartieACommence()) {
						System.out
								.println("Vous ne pouvez pas rejoindre la partie, elle a déjà commencé.");
						return null;
					}
	 ...
	 * @throws JoueurException
	 */
	public Partie rejoindrePartie(Partie aPartie, SantiagoInterface si)
			throws RemoteException, PartieException, JoueurException {
		System.out.println("coucou");
		this.verifieServer();
	...
	public int joueurFaitUneOffre() throws RemoteException {
		...
		if (offre < 0) {
			...
			System.out
					.println("Vous ne pouvez pas faire une offre inférieur à 0");

		} else if (offre > joueur.getSolde()) {
			envoyerMessage("Vous ne pouvez pas faire une offre supérieur à votre solde : "
					+ joueur.getSolde() + ".");
			System.out
					.println("Vous ne pouvez pas faire une offre supérieur à votre solde : "
							+ joueur.getSolde() + ".");
		} else {
			offreOk = true;
		}
	...
	public int propositionPhase4() throws RemoteException {
		...
		while (!choixValide) {
			System.out
					.println("[1] Je souhaite proposer un pot de vin au constructeur de canal pour construire mon canal");
			System.out
					.println("[2] Je souhaite soutenir la proposition de pot de vin d'un joueur");
	...
	/**
	 * Fonction appelée pour le choix 1 Demande un pot de vin et vérifier si le
		...
		while (!montantValide) {
			...
			try {
				...
				if (montant > 0 && joueur.getSolde() - montant >= 0) {
					...
					System.out
							.println("Votre solde ne peut être inférieur à 0 ! Montant maximum accepté: "
									+ joueur.getSolde() + " Escudos.");
					envoyerMessage("Votre solde ne peut être inférieur à 0 ! Montant maximum accepté: "
							+ joueur.getSolde() + " Escudos.");
				}
			} catch (InputMismatchException ex) {
				System.out.println("Erreur dans la valeur. \n");
	...
	/**
	 * Afficher la liste des propositions de pot de vin (Avec cumul des pots de
	 * vin)
	 * 
	 */
	@Override
	public void afficherPropositionsPotDeVin(
			HashMap<SantiagoInterface, Integer> listePropositions)
			throws RemoteException {
		this.verifieClient();

		for (SantiagoInterface si : listePropositions.keySet()) {
			System.out.println("Joueur : " + si.getJoueur().getPseudo()
					+ " | Couleur : " + si.getJoueur().getCouleur()
					+ " | Montant proposé: " + listePropositions.get(si));
			envoyerMessage("Joueur : " + si.getJoueur().getPseudo()
					+ " | Couleur : " + si.getJoueur().getCouleur()
					+ " | Montant proposé: " + listePropositions.get(si));
		}
	}
	...
	/**
	 * Fonction appelée pour le choix 2 Affiche la liste des propositions
	 * (Fonction afficherPropositionsPotDeVin) Demande de choisir un joueur dans
	 * la liste
	 * 
	 * @return SantiagoInterface: joueur soutenu
	 */
	public SantiagoInterface soutenirJoueur(
			HashMap<SantiagoInterface, Integer> listePropositions)
			throws RemoteException {
		this.verifieClient();
		...
		while (!choixJoueur) {
			if (choix.contains("")) {
				...
				System.out
						.println("Ce joueur n'est pas dans la liste des pots de vin.. Réessayez :");
				System.out.println("Quel joueur voulez vous soutenir ?");
	...
	/**
	 * Fonction appelée à la suite du soutien d'un joueur Cette fonction à pour
	 * rôle de cumuler le pot de vin du joueur au pot de vin du joueur soutenu
	 * Actualiste la liste des propositions
	 */
	@Override
	public void cumulerPotDeVin(
			HashMap<SantiagoInterface, Integer> listePropositions,
			SantiagoInterface joueurSoutenu, int potDeVin)
			throws RemoteException {
		this.verifieClient();

		for (SantiagoInterface si : listePropositions.keySet()) {
			if (si.equals(joueurSoutenu)) {
				listePropositions.replace(si, listePropositions.get(si)
						+ potDeVin);
				break;
			}
		}

		System.out.println("Votre pot de vin a été cumulé au pot de vin de "
				+ joueurSoutenu.getJoueur().getPseudo());
		envoyerMessage("Votre pot de vin a été cumulé au pot de vin de "
				+ joueurSoutenu.getJoueur().getPseudo());
	}

	/**
	 * Fonction appelée par le constructeur de canal Cette fonction appelle la
	 * fonction(constructeurPoserCanalPerso) qui retourne le coût de poser son
	 * propre canal Si le constructeur à un solde suffisant alors le choix lui
	 * est proposé
	 * 
	 * Cas 1: Appelle la fonction soutenirJoueur qui affiche la liste des
	 * propositions et retourne un joueur choisi Ajoute le pot de vin au solde
	 * du constructeur de canal
	 * 
	 ...
	 */
	@Override
	public SantiagoInterface choisirPotDeVin(Plateau plateau,
			HashMap<SantiagoInterface, Integer> listePropositions,
			ArrayList<Canal> listeCanauxTemp) throws RemoteException {
		this.verifieClient();
		...
		// Fonction pour vérifier si le constructeur a la possibilité de poser
		// son propre canal
		...
		while (!choixOk) {
			if (coutCanalPerso + 1 < joueur.getSolde()) {
				...
				System.out
						.println("[2] Choisir de constuire son propre canal (Coût: Proposition la plus élevée + 1 Escudo");
				...
				System.out
						.println("Vous n'avez pas un solde suffisant pour poser votre propre canal.");
				System.out
						.println("Vous devez donc poser le canal d'un joueur :");
			...
			case 1:
				...
				for (SantiagoInterface si : listePropositions.keySet()) {
					if (si.equals(joueurChoisi)) {
						try {
							joueur.setSolde(joueur.getSolde()
									+ listePropositions.get(si));

							// on met le canal en eau:
							for (Canal c : plateau.getListeCanaux()) {
								if (c.getCouleur().equals(
										joueurChoisi.getJoueur().getCouleur())) {
									plateau.metCanal(c);
								}
							}
			...
			case 2:
				choixOk = true;
				System.out
						.println("Vous avez décidé de poser votre propre canal. Son coût est de "
								+ (coutCanalPerso + 1) + " Escudos");
				envoyerMessage("Vous avez décidé de poser votre propre canal. Son coût est de "
						+ (coutCanalPerso + 1) + " Escudos");
				joueurChoisi = this;
	...
	/**
	 * Cette fonction est appelé à la fin de la phase 4 Si le constructeur de
	 * canal a choisi de construire son propre canal, ne fait rien Sinon
	 * parcourt la liste des soutiens pour leur déduire le pot de vin de soutien
	 * Puis déduit le pot de vin du joueur qui a créé la proposition de base
	 */
	@Override
	public void deduirePotDeVin(
			HashMap<SantiagoInterface, Integer> listePropositions,
			HashMap<SantiagoInterface, SantiagoInterface> listeSoutiens,
			HashMap<SantiagoInterface, Integer> listePotsDeVin,
			SantiagoInterface constructeur) throws RemoteException {
		this.verifieClient();
		...
		if (!this.equals(constructeur)) {
			...
			for (SantiagoInterface si : listePotsDeVin.keySet()) {
				if (si.equals(this)) {
					try {
						si.getJoueur().setSolde(
								si.getJoueur().getSolde()
										- listePotsDeVin.get(si));
					} catch (RemoteException | JoueurException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
	...
	/**
	 * Calcule le cout pour le constructeur de poser son canal perso (Basé sur
	 * la proposition max + 1)
	 * 
	 * @param listePropositions
	 * @return int: propositionMax
	 */
	public int constructeurPoserCanalPerso(
			HashMap<SantiagoInterface, Integer> listePropositions) {
		this.verifieClient();
	...
	/**
	 * Permet aux joueurs de poser un canal temporaire Canal : Couleur du joueur
	 * Mise en eau = false
	 */
	@Override
	public Canal poserCanalTemporaire(Plateau plateau,
			ArrayList<Canal> listeCanauxTemp) throws RemoteException {
		this.verifieClient();

		// TODO Auto-generated method stub
		System.out
				.println("Positionnez votre canal temporaire sur le plateau...");
		...
		while (!canalValide) {
			...
			for (Canal c : plateau.getListeCanaux()) {
				if (c.getCoordDebut().equals(canal.getCoordDebut())) {
					if (c.getCoordFin().equals(canal.getCoordFin())) {
						// La position est valide, on vérifie que le canal
						// n'existe pas déjà (!!! Pas sur que ça serve, à avoir
						// avec l'interface)
						canalValide = true;

						for (Canal c2 : listeCanauxTemp) {
							if (c2.getCoordDebut()
									.equals(canal.getCoordDebut())) {
								if (c2.getCoordFin()
										.equals(canal.getCoordFin())) {
									canalValide = false;
									break;
								}
							}
			...
			if (canalValide == true) {
				System.out
						.println("Canal validé. Au tour des autres joueurs...");
	...
	public void afficheErreur(String message) throws RemoteException {
		...
		// On enregistre également le pseudo pour que personne ne prenne le même
		// pseudo
	 ...
	 * @return : void
	 */
	public void enregistrePseudoEtMDP(String pseudo, String mdp)
			throws RemoteException {
		this.verifieServer();
	...
	/**
	 * Fonction qui vérifie si une connexion est possible avec un pseudo et un
	 * mdp
	 * 
	 * @params : String pseudo
	 ...
	 * @return : boolean
	 */
	public boolean connexionPseudoEtMDP(String pseudo, String motDePasse)
			throws RemoteException {
		this.verifieServer();
	...
	/**
	 * Fonction qui test si une partie est prête à être lancée. Si une partie
	 * est prête à être lancée, on lance la partie, Sinon on ne fait rien
	 * 
	 ...
	 * @throws JoueurException
	 */
	public void testPartieEstPrete(Partie aP) throws PartieException, RemoteException,
			JoueurException {
		this.verifieServer();
		 ...
		 * 
		 * - Que le nombre de rejoueur requis soit atteint - Qu'elle n'ait pas
		 * déjà commencé (bien évidemment)
		 */
	...
	/**
	 * Fonction appelée par le serveur uniquement pour mettre à jour la liste
	 * des parties Appelé lorsque le client appelle la fonction creerPartie().
	 * 
	 * @throws InterruptedException
	 */
	...
	/**
	 * Fonction appelée par le client pour voir la liste des parties en cours
	 * sur le serveur
	 * 
	...
	/**
	 * Fonction appelée par le client qui a créé la partie pour sauvegarder la
	 * partie en cours
	 * 
	 * @throws IOException
	...
	public void sauvegarder(Partie p) throws RemoteException,
	 ...
	 */
	public boolean reprendrePartie(Partie partieRejointe)
			throws RemoteException {
		this.verifieServer();
